...
CheckforSeparation::CheckforSeparation (double threshold) : separation_threshold (
    threshold)
{
}
...
void CheckforSeparation::CheckforViolations () const
{
  ...
  for (size_t i = 0; i < aircrafts.size (); ++i)
  {
    ...
    {
      double distance = CalculateDistance (aircrafts[i].Get_displacement(),
                                           aircrafts[j].Get_displacement());
      if (distance < separation_threshold)
      {
        std::cout << "Separation violation detected between "
            << aircrafts[i].Get_aircraft_id () << " and "
            << aircrafts[j].Get_aircraft_id () << " - Distance: " << distance
...
double CheckforSeparation::CalculateDistance (const Vectors &pos1,
                                              const Vectors &pos2) const
{
